...
async function testColumn(table, column) {
  ...
  if (error && error.code === 'PGRST204') {
    ...
    // RLS error means it passed schema validation!
    console.log(`${table} HAS column: ${column}`);
  } else {
    console.log(`${table} test for ${column} result:`, error);
  }
}

async function run() {
  const eventCols = ['id', 'title', 'date', 'time', 'location', 'description', 'category', 'isSabbathEve', 'imageurl', 'imageupload', 'documenturl'];
  console.log("Checking events...");
  for (const col of eventCols) {
    await testColumn('events', col);
  }
}
run();
