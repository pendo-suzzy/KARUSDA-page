...
async function test() {
  const { data, error } = await supabase
    .from('missions')
    .upsert({ id: 'test-123', title: 'Test', year: '2023', summary: 'Test', goalKes: 0, raisedKes: 0, upcoming: true }, { onConflict: ["id"] });
  
  if (error) {
    console.error("Error inserting into missions:", error);
  } else {
    console.log("Success inserting into missions:", data);
  }
}
test();
